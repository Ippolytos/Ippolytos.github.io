...
(function(){
  ...
  style.textContent=`
    @import url('https://fonts.googleapis.com/css2?family=Licorice&family=Cormorant+Garamond:ital,wght@1,400&family=Inter:wght@400;500&display=swap');
    ...
    .yy-tagline{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:17px;letter-spacing:2px;opacity:0;animation:yyFadeUp 1s cubic-bezier(.22,1,.36,1) 0.7s forwards;}
    .yy-sub{font-family:'Inter',sans-serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;opacity:0;animation:yyFadeUp 1s cubic-bezier(.22,1,.36,1) 1.05s forwards;}
...
(function(){
  ...
  btn.addEventListener('click',()=>{
    if(!playing){
      audio.play().then(()=>{
        btn.classList.add('vibing');
        lbl.textContent='jazz';
        playing=true;
      }).catch(()=>{
        lbl.textContent='lo-fi';
      });
    }else{
      audio.pause();
      btn.classList.remove('vibing');
      lbl.textContent='lo-fi';
      playing=false;
    }
  });
